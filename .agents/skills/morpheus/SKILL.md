...
7. This skill is **general-purpose**. Use it for paper recreation, model translation, hypothesis-driven modeling, and exploratory Morpheus work; do not assume a benchmark-only workflow.
...
When the task is based on a paper, PDF, screenshot, or prior Morpheus run, ground your work in **all available sources**:

- Read extracted paper text for mechanisms, entities, parameters, and stated outcomes.
- Inspect page images or figure images when they are available; do not rely on text extraction alone for visual phenotypes.
- Inspect representative Morpheus output images before deciding whether the model is behaving plausibly.
- Use references as structural templates, not as generic inspiration.
...
### Step 6: Review Outputs When Available

If the task includes simulation outputs:

- Compare early and late Morpheus frames, not just a single snapshot.
- Use image evidence to decide whether the model should be revised.
- If the visual behavior is clearly inconsistent with the stated biological phenomenon, revise the model conservatively and re-run.
...
### Paper and Figure Analysis

- Use figures to recover qualitative targets such as clustering, traveling fronts, stripes, spots, gradients, lumen formation, or invasion patterns.
- Use captions and surrounding paper text together; either one alone can be misleading.
- If a paper uses a different simulator or mathematical formalism, translate the biology and observables first, then adapt the closest Morpheus reference model.
