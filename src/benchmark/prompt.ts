...
export function buildCyclePrompt(params: PromptParams): string {
  ...
  const dependencyBlock = [
    ...
    "2. search_model_examples(...) or suggest_model_examples_for_paper(...) to find closest Morpheus corpus examples",
    "3. read_model_example_section(...) for the selected examples; read full XML only when a section is insufficient",
    "4. list_references(...) and read_reference(...) only as the legacy curated fallback",
    "5. write_model_xml(...)",
    "6. run_morpheus_model(...)",
    "7. summarize_morpheus_run(...)",
    "8. evaluate_technical_run(...)",
